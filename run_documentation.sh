...
cp README.md pythonbq/__init__.py
sed -i '1s/^/\"\"\"\n/' pythonbq/__init__.py
echo "\"\"\"" >> pythonbq/__init__.py
echo "from .pythonbq import pythonbq" >> pythonbq/__init__.py
...
function generate_docs() {
    ...
    if [ $INPUT_VERSION != "./" ]; then
        if [ $INPUT_VERSION != $VERSION ]; then
            pip install "./dist/pythonbq-$INPUT_VERSION.tar.gz"
        fi
    fi
    pdoc -o ./docs/$INPUT_VERSION -t ./doc_template pythonbq
}
